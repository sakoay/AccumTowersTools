%% Initialization code for optical sensor + Arduino + MouseReader_1sensor class setup.
%
%   This code initializes a MouseReader_1sensor object to be stored in the ViRMEn vr struct, which
...
%
function vr = initializeArduino_1sensor(vr, virmenDisplacementPerCm, virmenRotationsPerRev)

  % initialize mouse communications via Arduino 
  vr.mr           = MouseReader_1sensor(RigParameters.arduinoPort);
  
  % Scale factors for various components of velocity
  dotsPerRev      = RigParameters.sensorDotsPerRev;
