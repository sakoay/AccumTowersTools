...
const int default_sensor_pin = 4; // front sensor
//const int default_sensor_pin = 10; // bottom sensor
...
int sensor_pin; // pin for selecting the sensor
byte junk;
...
void selectSensor(int pin) {
   // set-up communication (Serial for PC; SPI for ADNS chip)
  sensor_pin = pin;
  Serial.begin(baud_rate);
  SPI.begin(sensor_pin);
  SPI.setBitOrder(sensor_pin,MSBFIRST);
  SPI.setDataMode(sensor_pin,SPI_MODE3);
  SPI.setClockDivider(sensor_pin,42); //84MHz clock on due; 2MHz on 3080);

  // Reset accumulated values since the sensor has changed
  delta_x = 0;
  delta_y = 0;
}
...
void setup() {
  ...
  // set-up communication (Serial for PC; SPI for ADNS chip)
  selectSensor(default_sensor_pin);
...
void loop() {
  ...
  if (t - t_last_poll >= poll_interval) {
    ...
    junk = SPI.transfer(sensor_pin,0x05,SPI_CONTINUE);
    delayMicroseconds(50);
    SQUAL= SPI.transfer(sensor_pin,0x00,SPI_CONTINUE);
    delayMicroseconds(50);
    
    junk = SPI.transfer(sensor_pin,0x02,SPI_CONTINUE);
    delayMicroseconds(50);
    MOTION = SPI.transfer(sensor_pin,0x00,SPI_CONTINUE);
    delayMicroseconds(80);
    
    junk = SPI.transfer(sensor_pin,0x03,SPI_CONTINUE);
    delayMicroseconds(50);
    x_motion = (signed char)SPI.transfer(sensor_pin,0x00,SPI_CONTINUE);
    delayMicroseconds(50);
    
    junk = SPI.transfer(sensor_pin,0x04,SPI_CONTINUE);
    delayMicroseconds(50);
    y_motion = (signed char)SPI.transfer(sensor_pin,0x00);
 ...
 // --- handle usb serial communication ---
  ...
  if (Serial.available()) {
    ...
    if (c == request_char) {
      // output integrated x/y displacement
      Serial.print(delta_x);
      Serial.print(separator_char);
