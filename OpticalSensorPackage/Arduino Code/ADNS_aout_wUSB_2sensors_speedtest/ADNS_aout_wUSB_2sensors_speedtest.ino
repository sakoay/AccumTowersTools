...
void loop() {
  ...
  if (t - t_last_poll >= poll_interval) {
    ...
    //------------------------------------------------------------
    // The following code is for testing the readout/transmission 
    // speed. Only one test (#1 or #2) should be used at any one
    // time, i.e. comment the other one out.
    
    // Test #1
    analogWrite(DAC0, map(testBit,0,1,0,4095));
    analogWrite(DAC1, map(testBit,0,1,0,4095));

    // Test #2
    analogWrite(DAC0, map(y_motion ,-10,10,0,4095));
    analogWrite(DAC1, map(y_motion2,-10,10,0,4095));
    //------------------------------------------------------------
